...
We gathered tweets from U.S. senators posted from August 16 to September 26, 2020. Our focus was on the 40 most active Twitter users among them, specifically selecting their top 10 tweets based on the highest combined counts of likes and retweets. Among these 40 senators, 24 are Democrats and 16 are Republicans.
...
From the network visualization, we can see that the 40 users are clustered into six groups, which will be used for following estimation of the stLDA-C model.
...
```
```{r include=FALSE}
tweets_stlda <- tweets_all %>% 
  mutate(text_com = ifelse(is.na(quoted_text), text, paste(text, quoted_text, sep=" "))) %>% 
  select(user_id, screen_name, text_com)
...
groundtruth_estimate <- collapsed_gibbs_1topic_clusters(alpha = 1, eta = 0.1, nu = 1,
...
Despite the advanced capabilities of the short text Latent Dirichlet Allocation (stLDA) model, obtaining ideal results from the analysis of 1200 tweets from 60 users proved challenging, primarily due to the inherent sparsity and brevity of tweets. These characteristics often result in limited contextual information and high variability in user-generated content, which can hinder the model's ability to accurately discern and categorize topics. Additionally, the effectiveness of stLDA is highly sensitive to text preprocessing choices and hyperparameter settings, which require meticulous tuning. To improve future analyses, a more extensive dataset could be beneficial, alongside a refined approach to preprocessing and an iterative process of parameter optimization to better capture the nuances of short text data.
